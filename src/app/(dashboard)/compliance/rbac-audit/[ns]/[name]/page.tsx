...
import type { RbacAuditDetail, Severity } from "@/types/compliance"
...
async function fetchDetail(ns: string, name: string): Promise<RbacAuditDetail | null> {
  try {
    const { getRbacAuditDetail } = await import("@/lib/compliance")
    return await getRbacAuditDetail(ns, name)
  } catch {
    return null
  }
}

export default async function RbacAuditDetailPage({ params }: Props) {
