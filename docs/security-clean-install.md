...
이 문서는 0부터 시작하는 클린 설치에서 보안 감사(`_workspace/security_audit_2026-05-10.md`) 결과를 빠짐없이 반영하기 위한 단계별 절차다. 자동화 가능한 부분은 `scripts/bootstrap-secrets.sh`가 담당하고, 외부 시스템(Keycloak/ArgoCD/OpenBao 등) 설정은 운영자가 직접 수행한다.
...
2. `Valid redirect URIs`: `https://portal.narwhal.local/api/auth/callback/keycloak`
...
**`.env.local`:**
```bash
KEYCLOAK_ISSUER=https://keycloak.narwhal.local/realms/narwhal
...
**`.env.local`:**
```bash
KEYCLOAK_URL=https://keycloak.narwhal.local
...
**절차:**
```bash
argocd login argocd.narwhal.local
argocd proj list
```
...
**`.env.local`:**
```bash
ARGOCD_URL=https://argocd.narwhal.local
...
```bash
TAG=0.1.0
docker build -t registry.narwhal.local/narwhal/tuning:$TAG narwhal/idp/manifests/tuning/
docker push registry.narwhal.local/narwhal/tuning:$TAG
docker images --digests registry.narwhal.local/narwhal/tuning | grep $TAG
# RELEASE@sha256:<64hex> 추출
```

**`.env.local`:**
```bash
TUNING_JOB_IMAGE=registry.narwhal.local/narwhal/tuning@sha256:<64hex>
...
**`.env.local`:**
```bash
OPENBAO_ADDR=https://openbao.narwhal.local:8200  # https:// 필수
...
|---|---|---|
| `AUTH_SECRET` | 1 | 자동 생성 |
| `AUTH_URL` | 수동 | `https://portal.narwhal.local` |
...
```bash
# CSP / 보안 헤더
curl -sI https://portal.narwhal.local | grep -iE 'strict-transport-security|content-security-policy|x-frame-options|x-content-type-options|referrer-policy|permissions-policy'

# 미인증 API 차단
curl -s -o /dev/null -w '%{http_code}' https://portal.narwhal.local/api/cluster
# → 401

# events/ingest 미인증 차단 (shared-secret 검증)
curl -s -o /dev/null -w '%{http_code}' -X POST https://portal.narwhal.local/api/events/ingest -d '{}'
# → 401

# /api/auth는 통과
curl -s -o /dev/null -w '%{http_code}' https://portal.narwhal.local/api/auth/session
# → 200 (세션 없음 응답)
```
